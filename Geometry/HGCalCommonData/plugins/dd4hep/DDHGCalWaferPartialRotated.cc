...
static long algorithm(dd4hep::Detector& /* description */, cms::DDParsingContext& ctxt, xml_h e) {
  ...
  edm::LogVerbatim("HGCalGeom") << "DDHGCalWaferPartialRotated: Module " << parentName << " made of " << material << " T "
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HGCalGeom") << "DDHGCalWaferPartialRotated: NameSpace " << ns.name() << " Sensitive Layer Name " << senseName
                                << " Thickness " << senseT << " Type " << senseType;
#endif
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HGCalGeom") << "DDHGCalWaferPartialRotated: " << solid.name() << " extruded polygon made of " << material
    ...
    for (unsigned int l = 0; l < layers.size(); l++) {
      ...
      if (layerType[i] > 0) {
        ...
        edm::LogVerbatim("HGCalGeom") << "DDHGCalWaferPartialRotated: " << solid.name() << " extruded polygon made of "
                                      ...
                                      << partialTypes[k] << " placement index " << placementIndex[m] << " and " << xL.size()
                                      << " edges";
        for (unsigned int j = 0; j < xL.size(); ++j)
          edm::LogVerbatim("HGCalGeom") << "[" << j << "] " << cms::convert2mm(xL[j]) << ":" << cms::convert2mm(yL[j]);
#endif
